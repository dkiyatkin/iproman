...
this.exec = function(cmd, argv, cb, logfile, v_stdout, v_stderr, this2) {
	...
	var stderr = '';
	cmd = spawn(cmd, argv);
	if (this3) { this2.emit('cmd', cmd); }
	cmd.stdout.on('data', function (data) {
		stdout = stdout + data;
		data = data.toString().trim()+'\n';
		if (logfile) { stream.write(data); }
		if (v_stdout) { process.stdout.write(data); }
		if (this3) { this3.emit('output', data, cmd); }
		if (this2 && logfile) { this2.emit('write_logfile', logfile, cmd); this2 = false; }
	});
